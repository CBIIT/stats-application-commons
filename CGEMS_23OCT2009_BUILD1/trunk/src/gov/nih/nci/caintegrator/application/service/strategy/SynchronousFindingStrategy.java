...
/**
 * This is a generic implementation of a FindingStrategy. It allows for a
 * QueryHandler to be set in it and will run a passed in QueryDTO against the
 * QueryHandler.
 * 
 * @author caIntegrator Team
 */
public class SynchronousFindingStrategy extends SessionBasedFindingStrategy {
    ...
    private static Logger logger = Logger
            .getLogger(SynchronousFindingStrategy.class);
    ...
    /**
     * This default constructor is necessary because we are using the strategies
     * as managed beans from spring. Spring needs a blank default construcctor.
     */
    public SynchronousFindingStrategy() {
        super(null, null);
    }
    ...
    /**
     * This executeQuery method can be called from anyone wanting to invoke the
     * entire strategy. First, it will call createQuery, then validate. It then
     * will call executeStrategy which is where the query will be sent to the
     * QueryHandler and results will be placed into the BusinessTierCache. It
     * will then call analyzeResultSet.
     * 
     * @return
    ...
    /**
     * The validate method will validate the query to make sure it is valid. It
     * also checks to see that the strategy is in the correct state and a
     * taskId, sessionId and queryDto have all been passed to it.
     * 
     * @param query
     * @return
    ...
    public boolean validate() throws ValidationException {
        if (getTaskResult().getTask().getCacheId() == null
                || getTaskResult().getTask().getId() == null
     ...
     * strategy. It will get the results from the QueryHandler back, and place
     * the "domain findings" into a taskResult. It then places the taskResult
     * finding into the cache. Runs the execute method in a separate thread for
    ...
    protected void executeStrategy() {
        ...
        try {
            findings = queryHandler.getResults(getTaskResult().getTask()
                    .getQueryDTO());
